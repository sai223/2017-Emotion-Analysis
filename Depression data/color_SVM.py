...
if __name__ == '__main__':
	...
	#SVM
	C_group = [1]
	Gamma_group = [0.001, 0.01, 0.1, ]

	for C in C_group:
		ml = SVC(kernel='rbf', C=C, random_state=0, probability=True)
		start_time = time()
		...
		print('C = ' + str(C))
		print('TEST NUM : %d, ERROR NUM : %d, ACCURACY : %.2f' %(len(Y_test), (Y_test != Y_pred).sum(), accuracy_score(Y_test, Y_pred)))
		print('\n')

	pickle.dump(ml, open('color_SVM.pkl', 'wb'))
