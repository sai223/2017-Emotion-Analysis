...
def CounterVector():
	...
	print("ACC : %.3f" %accuracy_score(Y_test, pred_logreg))
...
def TfIdf():
	pipe = make_pipeline(TfidfVectorizer(min_df=5), LogisticRegression(C=10))
	param_grid = {'logisticregression__C': [0.001, 0.01, 0.1, 1, 10]}

	logreg = pipe.fit(DB_X_train, Y_train)

	pred_logreg = logreg.predict(DB_X_test)
